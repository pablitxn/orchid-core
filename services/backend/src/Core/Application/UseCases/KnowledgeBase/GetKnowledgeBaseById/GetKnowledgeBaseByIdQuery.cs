...
namespace Core.Application.UseCases.KnowledgeBase.GetKnowledgeBaseById;

public record GetKnowledgeBaseByIdQuery(
    Guid Id,
    Guid UserId
) : IRequest<KnowledgeBaseItemDto>;

public class GetKnowledgeBaseByIdHandler : IRequestHandler<GetKnowledgeBaseByIdQuery, KnowledgeBaseItemDto>
{
    private readonly IKnowledgeBaseFileRepository _repository;

    public GetKnowledgeBaseByIdHandler(IKnowledgeBaseFileRepository repository)
    {
        _repository = repository;
    }

    public async Task<KnowledgeBaseItemDto> Handle(
        GetKnowledgeBaseByIdQuery request,
